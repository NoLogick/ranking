...
class EloRanker(TimeSeriesRanker):
    ...
    def leaderboard(self, method="min"):
        ...
        rtn['rating'] = rtn['rating'].replace([np.inf, -np.inf, np.nan], 0)
...
class TrueSkillRanker(TimeSeriesRanker):
    ...
    @staticmethod
    def wt(t, a):
        return TrueSkillRanker.vt(t, a) ** 2 + ((a - t) * pdf(a - t) + (a + t) * pdf(a + t)) / (
                    cdf(a - t) - cdf(- a - t))
    ...
    def update_single(self, host, visit, hscore, vscore, time=""):
        ...
        if abs(hscore - vscore) <= drawMargin:
            ...
            self.indexMiuLut[ih] += b * sh * v(b * (mh - mv) / cs ** (1 / 2), realDrawMargin / cs ** (1 / 2)) / cs ** (
                        1 / 2)
            self.indexMiuLut[iv] -= b * sv * v(b * (mv - mh) / cs ** (1 / 2), realDrawMargin / cs ** (1 / 2)) / cs ** (
                        1 / 2)
        ...
        return (
        self.rd * (self.indexMiuLut[ih] - 1.96 * self.indexSigmaSqrLut[ih] ** (1 / 2)) + (self.baseline - 3 * self.rd),
        self.rd * (self.indexMiuLut[iv] - 1.96 * self.indexSigmaSqrLut[iv] ** (1 / 2)) + (self.baseline - 3 * self.rd))
    ...
    def leaderboard(self, method="min"):
        ...
        rtn = pd.DataFrame({
            "name": self.data.indexlut,
            "rating": [self.rd * (i - 1.96 * j ** (1 / 2) - 3) + self.baseline for (i, j) in
...
class GlickoRanker(TimeSeriesRanker):
    ...
    def volatility_iter(self, deltaSqr, phiSqr, sigma, votality):
        ...
        f = lambda x: math.exp(x) * (deltaSqr - phiSqr - votality - math.exp(x)) / 2 / (
                    phiSqr + votality + math.exp(x)) ** 2 - (x - math.log(sigma ** 2)) / tau ** 2
        A = 2 * math.log(sigma)
    ...
    def update_single_batch(self, dataFrame):
        ...
        }, columns=['host', 'visit', 'hscore', 'vscore', 'time']).reset_index(drop=True).sort_values(
            by=['time', 'host'])
